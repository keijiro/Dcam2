...
public sealed partial class Flipbook
{
    ...
    async Awaitable InitObjects()
    {
        ...
        _bgParams.rparams = new RenderParams(_pageMaterial){ matProps = _bgParams.props, layer = gameObject.layer };
    ...
    async Awaitable RunSDPipelineAsync()
    {
        if (_sdPipeline != null)
        {
            _sdPipeline.Prompt = Prompt;
            _sdPipeline.Strength = Strength;
            _sdPipeline.GuidanceScale = Guidance;
